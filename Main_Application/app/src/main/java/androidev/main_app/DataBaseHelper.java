//This class is used to handle all requests to the database and
// stores any results returned from the database.
...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    //This method allows the removal of a user from the user table given the users ID.
    ...
    //This method allows the removal of all voting history for a user from the results table when given the user ID
    ...
    //This method allows the removal of all favourite it/saved pill for a user from the saved poll table given the user ID
    ...
    //This method allows the retrieval of poll data from the poll table when
    // given the poll ID from the saved poll table. The main required argument is the users ID.
    ...
    //This method allows the retrieval of poll data for when i user has completed a poll
    // but has not submitted it, from the poll table when
    // given the poll ID from the saved poll table. The main required argument is the users ID.
    public List<PollModel> getCompletedPolls(String UserID) {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.rawQuery("SELECT * FROM POLL_TABLE WHERE ID IN (SELECT POLL_ID FROM SAVED_POLL_TABLE WHERE USER_ID= ? and COMPLETE = 1)", new String[]{UserID});
    ...
    //Fetch only the polls that have a certain <String> value in the TOPIC column
    // from the results table, and store it
    // as objects using the methods from the "PollModel" class.
    public List<ResultsModel> getResultsPollInterest(String interest, String ID) {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = db.rawQuery("Select * from RESULTS_TABLE where TOPIC=? and USER_ID =?", new String[]{interest, ID});
    ...
    //Find the postcode of a user when given the users ID
    // and store it in a string which is then returned.
    ...
    //This method checks to see if a question has been voted on from the
    //votes table when given the question ID and the user ID.
    ...
    //This method checks to see if a result has been generated for a
    // specific poll and user from the results table when given the poll title and user Id
    public Boolean CheckResult(String pollTitle,String userID){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("Select * from RESULTS_TABLE where TITLE=? and USER_ID=?", new String[]{pollTitle,userID});
        if (cursor.getCount() >0){
            cursor.close();
            return true;
        }
    ...
    //Checks how many rows in the admin table contains
    //then return a true or false depending on the amount of rows
    ...
    //This method is used to insert a completed poll with a generated vote
    // into the results table. If the data is not inserted a false Boolean is returned.
    ...
    //This method is used to insert are voted on question into the votes
    // table a false bullying is returned if the insertion is not successful.
    ...
    //This method is used to insert poll data into the saved poll table when a user saves a
    // poll, Given the poll ID, the user ID and a Boolean for if the poll has been completed.
